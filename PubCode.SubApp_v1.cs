// !! FChecksum: 96B4025F667EAC35CC022AC0FD8E1FB5C50E8524AB01F55C6FC2CE9354913E26274AD4A408D5A991FFB1791804D685910CD9222172D059879DF16DD12DF64B4C
...
namespace PubCode.SubApp_v1 //v2021.4.25.1
{
    class SubAppAttribute : Attribute
    {
        ...
        public static void RunAppropriateApp(string[] args, ConsoleMgr myConsole) => RunAppropriateApp(null, args, myConsole);
        public static void RunAppropriateApp(string defaultApp, string[] args, ConsoleMgr myConsole)
        {
            ...
            foreach (var type in constructors_NotDefault)
                if (!done)
                    done = (bool)type.GetMethod("Try").Invoke(null, new object[] { false, args, myConsole });
            foreach (var type in constructors_Default)
                if (!done)
                    done = (bool)type.GetMethod("Try").Invoke(null, new object[] { true, args, myConsole });
            if (!done)
